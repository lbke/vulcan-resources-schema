export const DataFeed = {
  label: "DataFeed",
  description: `A single feed providing structured information about one or more entities or
topics.`,
  _id: {
    viewableBy: ["guests"],
    editableBy: ["members"],
...
school/university, a club, or a team.`,
  ...
  catalog: {
    ...
    resolveAs: {
      fieldName: "DataCatalogResolved",
      type: "DataCatalog",
      resolver: (document, args, context) => {
        return context.DataCatalog.findOne(
          { _id: document.DataCatalog },
          {
            fields: context.DataCatalog.getViewableFields(
              context.currentUser,
              context.DataCatalog
            ),
          }
  ...
  datasetTimeInterval: {
    ...
    description: `The range of temporal applicability of a dataset, e.g. for a 2011 census
dataset, the year 2011 (in ISO 8601 time interval format).`,
  ...
  spatial: {
    label: "spatial",
    description: `The range of spatial applicability of a dataset, e.g. for a dataset of New York
weather, the state of New York.`,
    resolveAs: {
      fieldName: "PlaceResolved",
      type: "Place",
      resolver: (document, args, context) => {
...
dataset, the year 2011 (in ISO 8601 time interval format).`,
  ...
  includedDataCatalog: {
    ...
    description: `A data catalog which contains this dataset (this property was previously
...
format.`,
    resolveAs: {
      fieldName: "DataDownloadResolved",
      type: "DataDownload",
      resolver: (document, args, context) => {
        return context.DataDownload.findOne(
          { _id: document.DataDownload },
          {
            fields: context.DataDownload.getViewableFields(
              context.currentUser,
              context.DataDownload
            ),
          }
        )
      },
